...
## Reading layer `PROVCenso2010' from data source `/home/aleir/unidad-2-asignacion-1-esda-operaciones-con-atributos-plots-aleira/data/divisionRD.gpkg' using driver `GPKG'
...
``` r
proven17 %>%
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿La salud?')) %>%
  plot(breaks = 'jenks')
```
...
#Rótulos: nombres de las provincias
proven17 %>%
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿La salud?'), TOPONIMIA) %>%
  gather(variable, valor, -geom, -TOPONIMIA) %>% 
  ggplot() + aes(fill = valor) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds'), trans = 'log10') +
...
``` r
proven17 %>% st_drop_geometry() %>%
  dplyr::select(TOPONIMIA, contains('Principales problemas de su barrio o comunidad: ¿La salud?'))
##                 TOPONIMIA
## 1       DISTRITO NACIONAL
## 2                    AZUA
## 3                 BAORUCO
...
proven17 %>%
  ...
  group_by(TOPONIMIA) %>% 
  mutate(pct=round(valor/sum(valor)*100,2)) %>% dplyr::select(-valor) %>% 
  filter(variable %in% 'Principales problemas de su barrio o comunidad: ¿La salud?: No') %>% 
  ggplot() + aes(fill = pct) + geom_sf(lwd = 0.2) +
  facet_wrap(~variable) + theme(text = element_text(size = 10)) +
  scale_fill_gradientn(colours = RColorBrewer::brewer.pal(9, name = 'Reds')) +
  geom_sf_text(aes(label=pct), check_overlap = T, size = 3)
...
proven17 %>% st_drop_geometry() %>%
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿La salud?')) %>%
  select_if(is.numeric) %>%  gather(variable, valor) %>%
  ggplot + aes(x=valor) %>% geom_histogram(bins=11) +
  scale_x_continuous(trans = 'log1p') + facet_grid(~variable)
```
...
``` r
proven17 %>% st_drop_geometry() %>% filter(ENLACE=='0320') %>% 
  dplyr::select(contains('Principales problemas de su barrio o comunidad: ¿La salud?'))
##   Principales problemas de su barrio o comunidad: ¿La salud?: Si
## 1                                                              9
##   Principales problemas de su barrio o comunidad: ¿La salud?: No
